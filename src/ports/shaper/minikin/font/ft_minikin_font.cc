...
constexpr FT_Int32 LOAD_FLAG = FT_LOAD_NO_HINTING | FT_LOAD_NO_BITMAP |
                               FT_LOAD_IGNORE_GLOBAL_ADVANCE_WIDTH;

constexpr float FTPosToFloat(FT_Pos x) { return x / 64.0; }
...
void loadGlyphOrDie(uint32_t glyphId, float size, FT_Face face) {
  ...
  FT_Set_Char_Size(face, scale, scale, 72 /* dpi */, 72 /* dpi */);
  FT_Load_Glyph(face, glyphId, LOAD_FLAG);

  //    LOG_ALWAYS_FATAL_IF(FT_Set_Char_Size(face, scale, scale, 72 /* dpi */,
  //    72 /* dpi */),
  //                        "Failed to set character size.");
  //    LOG_ALWAYS_FATAL_IF(FT_Load_Glyph(face, glyphId, LOAD_FLAG), "Failed to
  //    load glyph"); LOG_ALWAYS_FATAL_IF(face->glyph->format !=
...
FreeTypeMinikinFont::FreeTypeMinikinFont(
    const std::string& font_path, int index)
  ...
  //    LOG_ALWAYS_FATAL_IF(FT_Init_FreeType(&mFtLibrary), "Failed to initialize
  //    FreeType");
  ...
  //    LOG_ALWAYS_FATAL_IF(FT_Open_Face(mFtLibrary, &args, index, &mFtFace),
  //    "Failed to open FT_Face");
  FT_Open_Face(mFtLibrary, &args, index, &mFtFace);
}
...
float FreeTypeMinikinFont::GetHorizontalAdvance(
    uint32_t glyphId, const MinikinPaint& paint,
    const FontFakery& /* fakery */) const {
  loadGlyphOrDie(glyphId, paint.size, mFtFace);
  return FTPosToFloat(mFtFace->glyph->advance.x);
}

void FreeTypeMinikinFont::GetBounds(
    MinikinRect* bounds, uint32_t glyphId, const MinikinPaint& paint,
    const FontFakery& /* fakery */) const {
  loadGlyphOrDie(glyphId, paint.size, mFtFace);
...
void FreeTypeMinikinFont::GetFontExtent(
    MinikinExtent* extent, const MinikinPaint& paint,
    const FontFakery& /* fakery */) const {
