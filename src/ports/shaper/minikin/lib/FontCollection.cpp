...
void FontCollection::init(
  ...
  MINIKIN_ASSERT(nTypefaces <= MAX_FAMILY_COUNT,
                 "Font collection may only have up to %d font families.",
                 MAX_FAMILY_COUNT);
#ifdef USE_CMAP_COVERAGE
...
uint32_t FontCollection::calcFamilyScore(
    uint32_t ch, uint32_t vs, FamilyVariant variant, uint32_t localeListId,
    const std::shared_ptr<FontFamily>& fontFamily) const {
  const uint32_t coverageScore =
      calcCoverageScore(ch, vs, localeListId, fontFamily);
  ...
  const uint32_t localeScore =
      calcLocaleMatchingScore(localeListId, *fontFamily);
  ...
  // are for locale score, then the last 1 bit is for variant score.
...
uint32_t FontCollection::calcCoverageScore(
    uint32_t ch, uint32_t vs, uint32_t localeListId,
    const std::shared_ptr<FontFamily>& fontFamily) const {
  const bool hasVSGlyph = (vs != 0) && fontFamily->hasGlyph(ch, vs);
  if (!hasVSGlyph && !fontFamily->hasGlyph(ch, 0)) {
  ...
  if ((vs == 0 || hasVSGlyph) &&
      (mFamilies[0] == fontFamily || fontFamily->isCustomFallback())) {
...
//
// If two locales in the requested list have the same locale score, the font
// matching with higher priority locale gets a higher score. For example, in the
// case the user requested locale list is "ja-Jpan,en-Latn". The score of for
// the font of "ja-Jpan" gets a higher score than the font of "en-Latn".
//
// To achieve score calculation with priorities, the locale score is determined
// as follows:
//   LocaleScore = s(0) * 5^(m - 1) + s(1) * 5^(m - 2) + ... + s(m - 2) * 5 +
//   s(m - 1)
// Here, m is the maximum number of locales to be compared, and s(i) is the i-th
// locale's matching score. The possible values of s(i) are 0, 1, 2, 3 and 4.
uint32_t FontCollection::calcLocaleMatchingScore(uint32_t userLocaleListId,
  ...
  const LocaleList& fontLocaleList =
      LocaleListCache::getById(fontFamily.localeListId());
...
uint32_t FontCollection::calcVariantMatchingScore(
    FamilyVariant variant, const FontFamily& fontFamily) {
  ...
  if (variant == FamilyVariant::DEFAULT &&
      familyVariant == FamilyVariant::COMPACT) {
...
const std::shared_ptr<FontFamily> FontCollection::getFamilyForChar(
    uint32_t ch, uint32_t vs, uint32_t localeListId, FamilyVariant variant,
    FontStyle style) {
#ifdef USE_CMAP_COVERAGE
  if (ch >= mMaxChar) {
    return mFamilies[0];
  ...
  for (size_t i = range.start; i < range.end; i++) {
    ...
    const uint32_t score =
        calcFamilyScore(ch, vs, variant, localeListId, family);
  ...
  if (bestFamilyIndex == -1) {
    FontStyle font_style(FontStyle::Weight::NORMAL, FontStyle::Slant::UPRIGHT);
    return system_font_callback_->matchFamilyStyleCharacter(nullptr, font_style,
                                                            nullptr, 0, ch);
    // TODO
    //        UErrorCode errorCode = U_ZERO_ERROR;
    //        const UNormalizer2* normalizer =
    //        unorm2_getNFDInstance(&errorCode); if (U_SUCCESS(errorCode)) {
    //            UChar decomposed[4];
    //            int len = unorm2_getRawDecomposition(normalizer, ch,
    //            decomposed, 4, &errorCode); if (U_SUCCESS(errorCode) && len >
    //            0) {
    //                int off = 0;
  ...
  for (size_t i = 0; i < mFamilies.size(); i++) {
    const std::shared_ptr<FontFamily>& family = mFamilies[i];
    const uint32_t score =
        calcFamilyScore(ch, vs, variant, localeListId, family);
    if (score == kFirstFontScore) {
      // If the first font family supports the given character or variation
  ...
  if (bestFamilyIndex == -1) {
    auto font_family =
        TTFontManager::RefDefault()->matchFamilyStyleCharacter(
            nullptr, style, nullptr, 0, ch);
    if (font_family != nullptr) mFamilies.emplace_back(font_family);
    return font_family == nullptr ? mFamilies[0] : font_family;
  }
  return mFamilies[bestFamilyIndex];
#endif
}

// Characters where we want to continue using existing font run for (or stick to
// the next run if they start a string), even if the font does not support them
// explicitly. These are handled properly by Minikin or HarfBuzz even if the
// font does not explicitly support them and it's usually meaningless to switch
// to a different font to display them.
static bool doesNotNeedFontSupport(uint32_t c) {
  return c == 0x00AD     // SOFT HYPHEN
         || c == 0x034F  // COMBINING GRAPHEME JOINER
         || c == 0x061C  // ARABIC LETTER MARK
         || (0x200C <= c &&
             c <= 0x200F)  // ZERO WIDTH NON-JOINER..RIGHT-TO-LEFT MARK
         || (0x202A <= c &&
             c <= 0x202E)  // LEFT-TO-RIGHT EMBEDDING..RIGHT-TO-LEFT OVERRIDE
         || (0x2066 <= c &&
             c <= 0x2069)  // LEFT-TO-RIGHT ISOLATE..POP DIRECTIONAL ISOLATE
...
static inline bool isCombining(uint32_t c) {
  return (ICU_WRAPPER_GET_GC_MASK(c) & U_GC_M_MASK) != 0;
}
...
std::vector<FontCollection::Run> FontCollection::itemize(
    U16StringPiece text, FontStyle style, uint32_t localeListId,
    FamilyVariant familyVariant, uint32_t runMax) {
  ...
  do {
    ...
    if (doesNotNeedFontSupport(ch)) {
      // Always continue if the character is a format character not needed to be
      // in the font.
      shouldContinueRun = true;
    } else if (lastFamily != nullptr &&
               (isStickyWhitelisted(ch) || isCombining(ch))) {
      // Continue using existing font as long as it has coverage and is
      // whitelisted.
      shouldContinueRun = lastFamily->hasGlyph(ch, 0);
    }

    if (!shouldContinueRun) {
      const std::shared_ptr<FontFamily>& family =
          getFamilyForChar(ch, isVariationSelector(nextCh) ? nextCh : 0,
                           localeListId, familyVariant, style);
      if (utf16Pos == 0 || family.get() != lastFamily) {
        ...
        if (utf16Pos != 0 &&
            ...
            family != nullptr && family->hasGlyph(prevCh, 0)) {
        ...
        if (lastFamily == nullptr) {
          // This is the first family ever assigned. We are either seeing the
          // very first character (which means start would already be zero), or
          // we have only seen characters that don't need any font support
          // (which means we need to adjust start to be 0 to include those
          // characters).
          start = 0;
        }
        result.push_back(
    ...
    // Stop searching the remaining characters if the result length gets runMax
    // + 2. When result.size gets runMax + 2 here, the run between [0, runMax)
    // was finalized. If the result.size() equals to runMax, the run may be
    // still expanding. if the result.size() equals to runMax + 2, the last run
    // may be removed and the last run may be exntended the previous run with
    // above workaround.
  ...
  if (lastFamily == nullptr) {
    // No character needed any font support, so it doesn't really matter which
    // font they end up getting displayed in. We put the whole string in one
    // run, using the first font.
    result.push_back({mFamilies[0]->getClosestMatch(style), 0,
                      static_cast<int>(string_size)});
  }

  if (result.size() > runMax) {
    // The itemization has terminated since it reaches the runMax. Remove last
    // unfinalized runs.
...
uint32_t FontCollection::getId() const { return mId; }
const std::shared_ptr<MinikinFont> FontCollection::GetFontById(int32_t id) {
  for (auto& family : mFamilies) {
    for (int k = 0; k < family->getNumFonts(); k++) {
      auto* font = family->getFont(k);
      if (font->typeface()->GetUniqueId() == id) return font->typeface();
    }
  }
  return nullptr;
}
