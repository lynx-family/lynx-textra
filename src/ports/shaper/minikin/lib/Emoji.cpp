...
bool isEmoji(uint32_t c) {
  return ttoffice::ICUWrapper::GetInstance().icu_hasBinaryProperty(c,
                                                                   UCHAR_EMOJI);
}
...
bool isEmojiModifier(uint32_t c) {
  ...
  return ttoffice::ICUWrapper::GetInstance().icu_hasBinaryProperty(
      c, UCHAR_EMOJI_MODIFIER);
