...
class FontCollection {
 public:
  ...
  std::vector<Run> itemize(U16StringPiece text, FontStyle style,
                           uint32_t localeListId, FamilyVariant familyVariant,
                           uint32_t runMax);
  ...
  const std::shared_ptr<MinikinFont> GetFontById(int32_t id);
 ...
 private:
  ...
  uint32_t calcFamilyScore(uint32_t ch, uint32_t vs, FamilyVariant variant,
                           uint32_t localeListId,
  ...
  static uint32_t calcVariantMatchingScore(FamilyVariant variant,
                                           const FontFamily& fontFamily);
