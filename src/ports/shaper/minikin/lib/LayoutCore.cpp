...
static inline void addHyphenToHbBuffer(const HbBufferUniquePtr& buffer, const HbFontUniquePtr& font,
    ...
    const uint32_t hyphen_char = 0x002D;
    for (size_t i = 0; i < size; i++) {
//        hb_buffer_add(buffer.get(), determineHyphenChar(chars[i], font.get()), cluster);
        hb_buffer_add_utf32(buffer.get(), &hyphen_char, 1, 0, 1);
    }
}
...
LayoutPiece::LayoutPiece(const U16StringPiece& textBuf, const Range& range, bool isRtl,
    ...
    mAdvances.reserve(count);  // Need zero filling.
...
//            }
            for (unsigned int i = 0; i < numGlyphs; i++) {
                ...
                mGlyphIndices.push_back(info[i].cluster);
                float xAdvance = HBFixedToFloat(positions[i].x_advance);
                mAdvances.push_back(xAdvance);
...
//            }
    ...
    mGlyphIndices.shrink_to_fit();
    mAdvances.shrink_to_fit();
    mAdvance = x;
}
