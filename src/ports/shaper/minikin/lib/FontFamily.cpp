...
#include "lib/CmapCoverage.h"
#include "lib/FamilyVariant.h"
#include "lib/HbUtils.h"
...
Font Font::Builder::build() {
  if (mIsWeightSet && mIsSlantSet) {
    ...
    return Font(std::move(mTypeface), FontStyle(mWeight, mSlant),
                prepareFont(mTypeface));
  }
...
// static
HbFontUniquePtr Font::prepareFont(
    const std::shared_ptr<MinikinFont>& typeface) {
  ...
  HbBlobUniquePtr blob(
      hb_blob_create(buf, size, HB_MEMORY_MODE_READONLY, nullptr, nullptr));
...
FontStyle Font::analyzeStyle(const HbFontUniquePtr& font) {
  ...
  int weight;
  bool italic;
  if (!::minikin::analyzeStyle(os2Table.get(), os2Table.size(), &weight,
                               &italic)) {
    return FontStyle();
  }
  // TODO: Update weight/italic based on fvar value.
  return FontStyle(static_cast<uint16_t>(weight),
                   static_cast<FontStyle::Slant>(italic));
}
...
FontFamily::FontFamily(uint32_t localeListId, FamilyVariant variant,
                       std::vector<Font>&& fonts, bool isCustomFallback)
...
static FontFakery computeFakery(FontStyle wanted, FontStyle actual) {
  ...
  // select fake bold.
  bool isFakeBold =
      wanted.weight() >= 600 && (wanted.weight() - actual.weight()) >= 200;
...
bool FontFamily::hasGlyph(uint32_t codepoint,
  ...
  if (vsIndex >= mCmapFmt14Coverage.size()) {
    // Even if vsIndex is INVALID_VS_INDEX, we reach here since INVALID_VS_INDEX
    // is defined to be at the maximum end of the range.
    return false;
  }
...
#else
  auto* font = getClosestMatch(FontStyle()).font;
  return font->typeface()->UnicodeToGlyph(codepoint, variationSelector) != 0;
#endif
}
...
std::shared_ptr<FontFamily> FontFamily::createFamilyWithVariation(
  ...
  return std::shared_ptr<FontFamily>(new FontFamily(
      mLocaleListId, mVariant, std::move(fonts), mIsCustomFallback));
}
