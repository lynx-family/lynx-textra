...
#ifndef SRC_TEXTLAYOUT_UTILS_HASHER_H_
#define SRC_TEXTLAYOUT_UTILS_HASHER_H_
 ...
 public:
  ...
  inline Hasher& updateString(const std::string& str) {
    ...
    for (i = 0; i < (size & -4); i += 4) {
      update((uint32_t)str[i] | ((uint32_t)str[i + 1] << 8) |
...
#endif  // SRC_TEXTLAYOUT_UTILS_HASHER_H_
